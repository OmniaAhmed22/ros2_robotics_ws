...
class idealController(Node):

    def __init__(self):
        super().__init__("ideal_controller")
        ...
        self.vel_sub_ = self.create_subscription(Twist, "main_controller/cmd_vel", self.velCallback, 10)
        ...
        self.odom_msg_.child_frame_id = "ideal_base_footprint"
        ...
        self.transform_stamped_.child_frame_id = "ideal_base_footprint"
    ...
    def jointCallback(self, msg):
        ...
        delta_fi_left = msg.position[1] - self.left_wheel_prev_pos_
        delta_fi_right = msg.position[0] - self.right_wheel_prev_pos_
        dt = Time.from_msg(msg.header.stamp) - self.prev_time_

        dt_sec = max(dt.nanoseconds / S_TO_NS, 1e-6) #to avoid division by zero error
        #The Duration object has an attribute .nanoseconds, which gives the time difference as an integer number 
       ...
       # update the previous positions
        ...
        omega_fi_right = delta_fi_right / dt_sec
        ...
        d_s = (self.wheel_radius_ * delta_fi_right + self.wheel_radius_ * delta_fi_left) / 2
        d_theta = (self.wheel_radius_ * delta_fi_right - self.wheel_radius_ * delta_fi_left) / self.wheel_separation_
        self.theta_ += d_theta
...
def main():
    ...
    ideal_controller = idealController()
    rclpy.spin(ideal_controller)
    
    ideal_controller.destroy_node()
    rclpy.shutdown()
